...
import { fetchGoogleCalendarFeed, type GoogleCalendarEvent } from '@/services/googleCalendar'

const STORAGE_FEED_KEY = 'nexo_gcal_feed_url'
const STORAGE_ENABLED_KEY = 'nexo_gcal_enabled'
const STORAGE_EVENTS_KEY = 'nexo_gcal_events_cache'
const STORAGE_LAST_SYNC_KEY = 'nexo_gcal_last_sync'

function loadCachedEvents(): GoogleCalendarEvent[] {
  try {
...
export const useGoogleCalendarStore = defineStore('googleCalendar', () => {
  ...
  const events = ref<GoogleCalendarEvent[]>(loadCachedEvents())
  ...
  async function sync(): Promise<void> {
    ...
    try {
      const parsed = await fetchGoogleCalendarFeed(feedUrl.value)
      events.value = parsed
      ...
      const msg = err instanceof Error ? err.message : 'Error al sincronizar el calendario de Google'
      error.value = msg
      throw err
    } finally {
      loading.value = false
