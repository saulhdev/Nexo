...
defineProps<{
  event: M365Event | null
}>()

const emit = defineEmits<{ close: [] }>()
const { t } = useI18n()

function formatEventSchedule(evt: M365Event) {
  const start = formatDate(evt.startDate)
  if (evt.isAllDay) {
    if (evt.endDate && evt.endDate !== evt.startDate) {
      return `${start} – ${formatDate(evt.endDate)} (${t('m365.allDay')})`
    }
    return `${start} (${t('m365.allDay')})`
  }
      ...
      <div class="w-full max-w-lg rounded-2xl border border-line bg-surface p-6 shadow-2xl">
        <header class="flex items-start justify-between gap-3 border-b border-line pb-4">
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2 mb-1.5">
              <span class="inline-flex items-center gap-1 rounded-md bg-[#0078D4]/10 px-2 py-0.5 text-[11px] font-bold text-[#0078D4]">
        ...
        <div class="mt-5 space-y-4 text-sm">
          ...
          <div class="flex items-start gap-3 rounded-xl bg-canvas p-3">
            <Clock class="size-4 shrink-0 text-[#0078D4] mt-0.5" />
            <div>
              <p class="text-xs font-semibold text-muted uppercase tracking-wider">{{ t('m365.schedule') }}</p>
          ...
          <div v-if="event.location" class="flex items-start gap-3 rounded-xl bg-canvas p-3">
            ...
            <div>
              <p class="text-xs font-semibold text-muted uppercase tracking-wider">{{ t('m365.location') }}</p>
          ...
          <div class="flex items-center gap-2 rounded-xl bg-canvas/60 p-3 text-xs text-muted">
            <Info class="size-4 shrink-0 text-[#0078D4]" />
            <span>{{ t('m365.readOnlyNote') }}</span>
          </div>
        </div>
