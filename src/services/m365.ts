...
/**
 * Parses raw iCalendar (.ics) string content into M365Event objects
 */
export function parseICalendar(icsContent: string): M365Event[] {
  ...
  let inEvent = false
  let currentEvent: Partial<M365Event> = {}
  ...
  for (const rawLine of lines) {
    ...
    if (line === 'BEGIN:VEVENT') {
      inEvent = true
      currentEvent = { id: `m365-${Math.random().toString(36).slice(2, 9)}` }
      continue
    }
    ...
    if (line === 'END:VEVENT') {
      if (inEvent && currentEvent.title && currentEvent.startDate) {
        events.push({
          id: currentEvent.id || `m365-${Date.now()}-${Math.random()}`,
    ...
    switch (propName) {
      case 'UID':
        currentEvent.id = `m365-${propValue}`
        break
      case 'SUMMARY':
...
/**
 * Fetches an iCal feed URL with CORS fallback strategy
 */
export async function fetchM365CalendarFeed(url: string): Promise<M365Event[]> {
  ...
  try {
    ...
    // Direct fetch failed (likely CORS restriction on Microsoft domain)
  }
  ...
  if (!textData || !textData.includes('BEGIN:VCALENDAR')) {
    ...
    if (!res.ok) {
      throw new Error(`No se pudo descargar el calendario de Microsoft 365 (Error ${res.status}).`)
    }
    textData = await res.text()
  }
